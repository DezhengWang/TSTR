...
from utils.embed import DataEmbeddingStatic
...
class Model(nn.Module):
    def __init__(self, enc_in, dec_in, c_out, out_len,
        ...
        # Encoding
        self.enc_embedding = DataEmbeddingStatic(enc_in, d_model, dropout)
        ...
        self.eco_att = AttentionLayer(StaticAttention(False, dropout=dropout,
                                                      atts=output_attention,n_size=(self.args.feature,
                                                                                    self.args.feature),
                                                      n_heads=n_heads),
                                          args.seq_len, n_heads, mix=False)

    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
                enc_self_mask=None, dec_self_mask=None, dec_enc_mask=None):

        x_enc, att_vari = self.eco_att(x_enc.transpose(1,2),
                                       x_enc.transpose(1,2),
                                       None)
